...
class Weather_block extends Component {
    ...
    createWeatherData = () => {
        this.Api.get_weather(this.props.data)
            .then((result) => {
                if (
                    result.firstDayWeather.current &&
                    result.secondDayWeather.current &&
                    result.thirdDayWeather.current !== undefined
                ) {
                    this.setState({
                ...
                this.setState({
                    chanceOfRain:
                        result.chanceOfRain.forecast.forecastday[0].hour[0].chance_of_rain,
                });
            });
    };

    renameCity() {
        let firstWordChar = this.props.data.split("").slice(0, 1)[0].toUpperCase();
        let wordRest = this.props.data.split("").slice(1).join("").toLowerCase();
        this.setState({
            cityCopy: firstWordChar + wordRest,
        });
    }
        ...
        return (
            <div className='weather_block'>
                <div className='weather_block_info__container'>
                    ...
                    <p className='weather_block_temp very_big_black_text'>
                        {this.state.firstDayWeather.temp_c}°
                    </p>
                </div>
                <div className='weather_block_icon__container'>
                    <img
                        className='weather_block_icon'
                        alt='weather_image'
                        src={this.firstConditionIcon}
                    />
                </div>
            </div>
        );
